...
/*******************************************************************************
* Function Name: SW2Debounce_clock_Start
********************************************************************************
*
* Summary:
...
void SW2Debounce_clock_Start(void) 
{
    ...
    SW2Debounce_clock_CLKEN |= SW2Debounce_clock_CLKEN_MASK;
	SW2Debounce_clock_CLKSTBY |= SW2Debounce_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SW2Debounce_clock_Stop
********************************************************************************
*
* Summary:
...
void SW2Debounce_clock_Stop(void) 
{
    ...
    SW2Debounce_clock_CLKEN &= (uint8)(~SW2Debounce_clock_CLKEN_MASK);
	SW2Debounce_clock_CLKSTBY &= (uint8)(~SW2Debounce_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SW2Debounce_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SW2Debounce_clock_CLKEN & SW2Debounce_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SW2Debounce_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SW2Debounce_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SW2Debounce_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SW2Debounce_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SW2Debounce_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SW2Debounce_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SW2Debounce_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SW2Debounce_clock_GetSourceRegister();
    uint16 oldDivider = SW2Debounce_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SW2Debounce_clock_CLKEN & SW2Debounce_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SW2Debounce_clock_DIV_PTR, clkDivider);
                SW2Debounce_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SW2Debounce_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SW2Debounce_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SW2Debounce_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(SW2Debounce_clock_DIV_PTR, clkDivider);
				SW2Debounce_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: SW2Debounce_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SW2Debounce_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: SW2Debounce_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SW2Debounce_clock_MOD_SRC |= modeBitMask & (uint8)SW2Debounce_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: SW2Debounce_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SW2Debounce_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SW2Debounce_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: SW2Debounce_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SW2Debounce_clock_MOD_SRC & (uint8)(SW2Debounce_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: SW2Debounce_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SW2Debounce_clock_GetDividerRegister();
    uint8 oldSrc = SW2Debounce_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SW2Debounce_clock_MOD_SRC |= CYCLK_SSS;
        SW2Debounce_clock_MOD_SRC =
            (SW2Debounce_clock_MOD_SRC & (uint8)(~SW2Debounce_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SW2Debounce_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SW2Debounce_clock_MOD_SRC & SW2Debounce_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SW2Debounce_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SW2Debounce_clock_PHASE = clkPhase & SW2Debounce_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SW2Debounce_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SW2Debounce_clock_PHASE & SW2Debounce_clock_PHASE_MASK;
}
