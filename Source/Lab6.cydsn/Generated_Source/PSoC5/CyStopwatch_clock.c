...
/*******************************************************************************
* Function Name: CyStopwatch_clock_Start
********************************************************************************
*
* Summary:
...
void CyStopwatch_clock_Start(void) 
{
    ...
    CyStopwatch_clock_CLKEN |= CyStopwatch_clock_CLKEN_MASK;
	CyStopwatch_clock_CLKSTBY |= CyStopwatch_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CyStopwatch_clock_Stop
********************************************************************************
*
* Summary:
...
void CyStopwatch_clock_Stop(void) 
{
    ...
    CyStopwatch_clock_CLKEN &= (uint8)(~CyStopwatch_clock_CLKEN_MASK);
	CyStopwatch_clock_CLKSTBY &= (uint8)(~CyStopwatch_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CyStopwatch_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CyStopwatch_clock_CLKEN & CyStopwatch_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CyStopwatch_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CyStopwatch_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CyStopwatch_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CyStopwatch_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CyStopwatch_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CyStopwatch_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CyStopwatch_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CyStopwatch_clock_GetSourceRegister();
    uint16 oldDivider = CyStopwatch_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CyStopwatch_clock_CLKEN & CyStopwatch_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CyStopwatch_clock_DIV_PTR, clkDivider);
                CyStopwatch_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CyStopwatch_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CyStopwatch_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CyStopwatch_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CyStopwatch_clock_DIV_PTR, clkDivider);
				CyStopwatch_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CyStopwatch_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CyStopwatch_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: CyStopwatch_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CyStopwatch_clock_MOD_SRC |= modeBitMask & (uint8)CyStopwatch_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: CyStopwatch_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CyStopwatch_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CyStopwatch_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: CyStopwatch_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CyStopwatch_clock_MOD_SRC & (uint8)(CyStopwatch_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: CyStopwatch_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CyStopwatch_clock_GetDividerRegister();
    uint8 oldSrc = CyStopwatch_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CyStopwatch_clock_MOD_SRC |= CYCLK_SSS;
        CyStopwatch_clock_MOD_SRC =
            (CyStopwatch_clock_MOD_SRC & (uint8)(~CyStopwatch_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CyStopwatch_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CyStopwatch_clock_MOD_SRC & CyStopwatch_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CyStopwatch_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CyStopwatch_clock_PHASE = clkPhase & CyStopwatch_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CyStopwatch_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CyStopwatch_clock_PHASE & CyStopwatch_clock_PHASE_MASK;
}
