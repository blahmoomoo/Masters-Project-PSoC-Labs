...
/*******************************************************************************
* Function Name: Stopwatch_clock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_Start(void) 
{
    /* Set the bit to enable the clock. */
    Stopwatch_clock_CLKEN |= Stopwatch_clock_CLKEN_MASK;
	Stopwatch_clock_CLKSTBY |= Stopwatch_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Stopwatch_clock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Stopwatch_clock_CLKEN &= (uint8)(~Stopwatch_clock_CLKEN_MASK);
	Stopwatch_clock_CLKSTBY &= (uint8)(~Stopwatch_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Stopwatch_clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_StopBlock(void) 
{
    if ((Stopwatch_clock_CLKEN & Stopwatch_clock_CLKEN_MASK) != 0u)
    {
...
#if defined(Stopwatch_clock__CFG3)
        CLK_DIST_AMASK = Stopwatch_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Stopwatch_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Stopwatch_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Stopwatch_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Stopwatch_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Stopwatch_clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Stopwatch_clock_CLKSTBY &= (uint8)(~Stopwatch_clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Stopwatch_clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Stopwatch_clock_GetSourceRegister();
    uint16 oldDivider = Stopwatch_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Stopwatch_clock_CLKEN & Stopwatch_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Stopwatch_clock_DIV_PTR, clkDivider);
                Stopwatch_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Stopwatch_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Stopwatch_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Stopwatch_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Stopwatch_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Stopwatch_clock_GetDividerRegister(void) 
{
    return CY_GET_REG16(Stopwatch_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Stopwatch_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_SetModeRegister(uint8 modeBitMask) 
{
    Stopwatch_clock_MOD_SRC |= modeBitMask & (uint8)Stopwatch_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Stopwatch_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_ClearModeRegister(uint8 modeBitMask) 
{
    Stopwatch_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Stopwatch_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Stopwatch_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Stopwatch_clock_GetModeRegister(void) 
{
    return Stopwatch_clock_MOD_SRC & (uint8)(Stopwatch_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Stopwatch_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Stopwatch_clock_GetDividerRegister();
    uint8 oldSrc = Stopwatch_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Stopwatch_clock_MOD_SRC |= CYCLK_SSS;
        Stopwatch_clock_MOD_SRC =
            (Stopwatch_clock_MOD_SRC & (uint8)(~Stopwatch_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Stopwatch_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Stopwatch_clock_GetSourceRegister(void) 
{
    return Stopwatch_clock_MOD_SRC & Stopwatch_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Stopwatch_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Stopwatch_clock_SetPhaseRegister(uint8 clkPhase) 
{
    Stopwatch_clock_PHASE = clkPhase & Stopwatch_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Stopwatch_clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Stopwatch_clock_GetPhaseRegister(void) 
{
    return Stopwatch_clock_PHASE & Stopwatch_clock_PHASE_MASK;
}
