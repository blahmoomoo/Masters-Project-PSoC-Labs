/*******************************************************************************
* File Name: CyStopwatch_clock2.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: CyStopwatch_clock2_Start
********************************************************************************
*
* Summary:
...
void CyStopwatch_clock2_Start(void) 
{
    ...
    CyStopwatch_clock2_CLKEN |= CyStopwatch_clock2_CLKEN_MASK;
	CyStopwatch_clock2_CLKSTBY |= CyStopwatch_clock2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CyStopwatch_clock2_Stop
********************************************************************************
*
* Summary:
...
void CyStopwatch_clock2_Stop(void) 
{
    ...
    CyStopwatch_clock2_CLKEN &= (uint8)(~CyStopwatch_clock2_CLKEN_MASK);
	CyStopwatch_clock2_CLKSTBY &= (uint8)(~CyStopwatch_clock2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CyStopwatch_clock2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CyStopwatch_clock2_CLKEN & CyStopwatch_clock2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CyStopwatch_clock2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CyStopwatch_clock2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CyStopwatch_clock2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CyStopwatch_clock2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CyStopwatch_clock2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CyStopwatch_clock2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CyStopwatch_clock2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CyStopwatch_clock2_GetSourceRegister();
    uint16 oldDivider = CyStopwatch_clock2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CyStopwatch_clock2_CLKEN & CyStopwatch_clock2_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CyStopwatch_clock2_DIV_PTR, clkDivider);
                CyStopwatch_clock2_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CyStopwatch_clock2_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CyStopwatch_clock2_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CyStopwatch_clock2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CyStopwatch_clock2_DIV_PTR, clkDivider);
				CyStopwatch_clock2_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CyStopwatch_clock2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CyStopwatch_clock2_DIV_PTR);
}


/*******************************************************************************
* Function Name: CyStopwatch_clock2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CyStopwatch_clock2_MOD_SRC |= modeBitMask & (uint8)CyStopwatch_clock2_MODE_MASK;
}


/*******************************************************************************
* Function Name: CyStopwatch_clock2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CyStopwatch_clock2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CyStopwatch_clock2_MODE_MASK));
}


/*******************************************************************************
* Function Name: CyStopwatch_clock2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CyStopwatch_clock2_MOD_SRC & (uint8)(CyStopwatch_clock2_MODE_MASK);
}


/*******************************************************************************
* Function Name: CyStopwatch_clock2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CyStopwatch_clock2_GetDividerRegister();
    uint8 oldSrc = CyStopwatch_clock2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CyStopwatch_clock2_MOD_SRC |= CYCLK_SSS;
        CyStopwatch_clock2_MOD_SRC =
            (CyStopwatch_clock2_MOD_SRC & (uint8)(~CyStopwatch_clock2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CyStopwatch_clock2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CyStopwatch_clock2_MOD_SRC & CyStopwatch_clock2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CyStopwatch_clock2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CyStopwatch_clock2_PHASE = clkPhase & CyStopwatch_clock2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CyStopwatch_clock2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CyStopwatch_clock2_PHASE & CyStopwatch_clock2_PHASE_MASK;
}
