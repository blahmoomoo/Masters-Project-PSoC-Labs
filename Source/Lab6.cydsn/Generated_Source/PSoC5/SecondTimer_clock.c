...
/*******************************************************************************
* Function Name: SecondTimer_clock_Start
********************************************************************************
*
* Summary:
...
void SecondTimer_clock_Start(void) 
{
    ...
    SecondTimer_clock_CLKEN |= SecondTimer_clock_CLKEN_MASK;
	SecondTimer_clock_CLKSTBY |= SecondTimer_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SecondTimer_clock_Stop
********************************************************************************
*
* Summary:
...
void SecondTimer_clock_Stop(void) 
{
    ...
    SecondTimer_clock_CLKEN &= (uint8)(~SecondTimer_clock_CLKEN_MASK);
	SecondTimer_clock_CLKSTBY &= (uint8)(~SecondTimer_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SecondTimer_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SecondTimer_clock_CLKEN & SecondTimer_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SecondTimer_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SecondTimer_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SecondTimer_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SecondTimer_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SecondTimer_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SecondTimer_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SecondTimer_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SecondTimer_clock_GetSourceRegister();
    uint16 oldDivider = SecondTimer_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SecondTimer_clock_CLKEN & SecondTimer_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SecondTimer_clock_DIV_PTR, clkDivider);
                SecondTimer_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SecondTimer_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SecondTimer_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SecondTimer_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(SecondTimer_clock_DIV_PTR, clkDivider);
				SecondTimer_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: SecondTimer_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SecondTimer_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: SecondTimer_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SecondTimer_clock_MOD_SRC |= modeBitMask & (uint8)SecondTimer_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: SecondTimer_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SecondTimer_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SecondTimer_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: SecondTimer_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SecondTimer_clock_MOD_SRC & (uint8)(SecondTimer_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: SecondTimer_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SecondTimer_clock_GetDividerRegister();
    uint8 oldSrc = SecondTimer_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SecondTimer_clock_MOD_SRC |= CYCLK_SSS;
        SecondTimer_clock_MOD_SRC =
            (SecondTimer_clock_MOD_SRC & (uint8)(~SecondTimer_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SecondTimer_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SecondTimer_clock_MOD_SRC & SecondTimer_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SecondTimer_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SecondTimer_clock_PHASE = clkPhase & SecondTimer_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SecondTimer_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SecondTimer_clock_PHASE & SecondTimer_clock_PHASE_MASK;
}
