/*******************************************************************************
* File Name: CyRefreshInterval_clock.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: CyRefreshInterval_clock_Start
********************************************************************************
*
* Summary:
...
void CyRefreshInterval_clock_Start(void) 
{
    ...
    CyRefreshInterval_clock_CLKEN |= CyRefreshInterval_clock_CLKEN_MASK;
	CyRefreshInterval_clock_CLKSTBY |= CyRefreshInterval_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CyRefreshInterval_clock_Stop
********************************************************************************
*
* Summary:
...
void CyRefreshInterval_clock_Stop(void) 
{
    ...
    CyRefreshInterval_clock_CLKEN &= (uint8)(~CyRefreshInterval_clock_CLKEN_MASK);
	CyRefreshInterval_clock_CLKSTBY &= (uint8)(~CyRefreshInterval_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CyRefreshInterval_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CyRefreshInterval_clock_CLKEN & CyRefreshInterval_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CyRefreshInterval_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CyRefreshInterval_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CyRefreshInterval_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CyRefreshInterval_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CyRefreshInterval_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CyRefreshInterval_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CyRefreshInterval_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CyRefreshInterval_clock_GetSourceRegister();
    uint16 oldDivider = CyRefreshInterval_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CyRefreshInterval_clock_CLKEN & CyRefreshInterval_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CyRefreshInterval_clock_DIV_PTR, clkDivider);
                CyRefreshInterval_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CyRefreshInterval_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CyRefreshInterval_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CyRefreshInterval_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CyRefreshInterval_clock_DIV_PTR, clkDivider);
				CyRefreshInterval_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CyRefreshInterval_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CyRefreshInterval_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: CyRefreshInterval_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CyRefreshInterval_clock_MOD_SRC |= modeBitMask & (uint8)CyRefreshInterval_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: CyRefreshInterval_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CyRefreshInterval_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CyRefreshInterval_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: CyRefreshInterval_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CyRefreshInterval_clock_MOD_SRC & (uint8)(CyRefreshInterval_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: CyRefreshInterval_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CyRefreshInterval_clock_GetDividerRegister();
    uint8 oldSrc = CyRefreshInterval_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CyRefreshInterval_clock_MOD_SRC |= CYCLK_SSS;
        CyRefreshInterval_clock_MOD_SRC =
            (CyRefreshInterval_clock_MOD_SRC & (uint8)(~CyRefreshInterval_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CyRefreshInterval_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CyRefreshInterval_clock_MOD_SRC & CyRefreshInterval_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CyRefreshInterval_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CyRefreshInterval_clock_PHASE = clkPhase & CyRefreshInterval_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CyRefreshInterval_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CyRefreshInterval_clock_PHASE & CyRefreshInterval_clock_PHASE_MASK;
}
