...
/*******************************************************************************
* Function Name: Reset_clock_Start
********************************************************************************
*
* Summary:
...
void Reset_clock_Start(void) 
{
    ...
    Reset_clock_CLKEN |= Reset_clock_CLKEN_MASK;
	Reset_clock_CLKSTBY |= Reset_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Reset_clock_Stop
********************************************************************************
*
* Summary:
...
void Reset_clock_Stop(void) 
{
    ...
    Reset_clock_CLKEN &= (uint8)(~Reset_clock_CLKEN_MASK);
	Reset_clock_CLKSTBY &= (uint8)(~Reset_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Reset_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Reset_clock_CLKEN & Reset_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Reset_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Reset_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Reset_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Reset_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Reset_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Reset_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Reset_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Reset_clock_GetSourceRegister();
    uint16 oldDivider = Reset_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Reset_clock_CLKEN & Reset_clock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Reset_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Reset_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Reset_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Reset_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Reset_clock_MOD_SRC |= modeBitMask & (uint8)Reset_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Reset_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Reset_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Reset_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Reset_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Reset_clock_MOD_SRC & (uint8)(Reset_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Reset_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Reset_clock_GetDividerRegister();
    uint8 oldSrc = Reset_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Reset_clock_MOD_SRC |= CYCLK_SSS;
        Reset_clock_MOD_SRC =
            (Reset_clock_MOD_SRC & (uint8)(~Reset_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Reset_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Reset_clock_MOD_SRC & Reset_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Reset_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Reset_clock_PHASE = clkPhase & Reset_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Reset_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Reset_clock_PHASE & Reset_clock_PHASE_MASK;
}
