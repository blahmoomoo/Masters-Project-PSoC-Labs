/*******************************************************************************
* File Name: CySYNTH_MASTER_CLK.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_Start
********************************************************************************
*
* Summary:
...
void CySYNTH_MASTER_CLK_Start(void) 
{
    ...
    CySYNTH_MASTER_CLK_CLKEN |= CySYNTH_MASTER_CLK_CLKEN_MASK;
	CySYNTH_MASTER_CLK_CLKSTBY |= CySYNTH_MASTER_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_Stop
********************************************************************************
*
* Summary:
...
void CySYNTH_MASTER_CLK_Stop(void) 
{
    ...
    CySYNTH_MASTER_CLK_CLKEN &= (uint8)(~CySYNTH_MASTER_CLK_CLKEN_MASK);
	CySYNTH_MASTER_CLK_CLKSTBY &= (uint8)(~CySYNTH_MASTER_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CySYNTH_MASTER_CLK_CLKEN & CySYNTH_MASTER_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CySYNTH_MASTER_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CySYNTH_MASTER_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CySYNTH_MASTER_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CySYNTH_MASTER_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CySYNTH_MASTER_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CySYNTH_MASTER_CLK_GetSourceRegister();
    uint16 oldDivider = CySYNTH_MASTER_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CySYNTH_MASTER_CLK_CLKEN & CySYNTH_MASTER_CLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CySYNTH_MASTER_CLK_DIV_PTR, clkDivider);
                CySYNTH_MASTER_CLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CySYNTH_MASTER_CLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CySYNTH_MASTER_CLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CySYNTH_MASTER_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CySYNTH_MASTER_CLK_DIV_PTR, clkDivider);
				CySYNTH_MASTER_CLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CySYNTH_MASTER_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CySYNTH_MASTER_CLK_MOD_SRC |= modeBitMask & (uint8)CySYNTH_MASTER_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CySYNTH_MASTER_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CySYNTH_MASTER_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CySYNTH_MASTER_CLK_MOD_SRC & (uint8)(CySYNTH_MASTER_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CySYNTH_MASTER_CLK_GetDividerRegister();
    uint8 oldSrc = CySYNTH_MASTER_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CySYNTH_MASTER_CLK_MOD_SRC |= CYCLK_SSS;
        CySYNTH_MASTER_CLK_MOD_SRC =
            (CySYNTH_MASTER_CLK_MOD_SRC & (uint8)(~CySYNTH_MASTER_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CySYNTH_MASTER_CLK_MOD_SRC & CySYNTH_MASTER_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CySYNTH_MASTER_CLK_PHASE = clkPhase & CySYNTH_MASTER_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CySYNTH_MASTER_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CySYNTH_MASTER_CLK_PHASE & CySYNTH_MASTER_CLK_PHASE_MASK;
}
