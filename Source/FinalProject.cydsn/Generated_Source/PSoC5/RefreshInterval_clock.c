/*******************************************************************************
* File Name: RefreshInterval_clock.c
* Version 2.10
*
*  Description:
...
#include "RefreshInterval_clock.h"
...
/*******************************************************************************
* Function Name: RefreshInterval_clock_Start
********************************************************************************
*
* Summary:
...
void RefreshInterval_clock_Start(void) 
{
    ...
    RefreshInterval_clock_CLKEN |= RefreshInterval_clock_CLKEN_MASK;
	RefreshInterval_clock_CLKSTBY |= RefreshInterval_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: RefreshInterval_clock_Stop
********************************************************************************
*
* Summary:
...
void RefreshInterval_clock_Stop(void) 
{
    ...
    RefreshInterval_clock_CLKEN &= (uint8)(~RefreshInterval_clock_CLKEN_MASK);
	RefreshInterval_clock_CLKSTBY &= (uint8)(~RefreshInterval_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: RefreshInterval_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((RefreshInterval_clock_CLKEN & RefreshInterval_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = RefreshInterval_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = RefreshInterval_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* RefreshInterval_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(RefreshInterval_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(RefreshInterval_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: RefreshInterval_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: RefreshInterval_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = RefreshInterval_clock_GetSourceRegister();
    uint16 oldDivider = RefreshInterval_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = RefreshInterval_clock_CLKEN & RefreshInterval_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(RefreshInterval_clock_DIV_PTR, clkDivider);
                RefreshInterval_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                RefreshInterval_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(RefreshInterval_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((RefreshInterval_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(RefreshInterval_clock_DIV_PTR, clkDivider);
				RefreshInterval_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: RefreshInterval_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(RefreshInterval_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: RefreshInterval_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    RefreshInterval_clock_MOD_SRC |= modeBitMask & (uint8)RefreshInterval_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: RefreshInterval_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    RefreshInterval_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(RefreshInterval_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: RefreshInterval_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return RefreshInterval_clock_MOD_SRC & (uint8)(RefreshInterval_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: RefreshInterval_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = RefreshInterval_clock_GetDividerRegister();
    uint8 oldSrc = RefreshInterval_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        RefreshInterval_clock_MOD_SRC |= CYCLK_SSS;
        RefreshInterval_clock_MOD_SRC =
            (RefreshInterval_clock_MOD_SRC & (uint8)(~RefreshInterval_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: RefreshInterval_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return RefreshInterval_clock_MOD_SRC & RefreshInterval_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: RefreshInterval_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    RefreshInterval_clock_PHASE = clkPhase & RefreshInterval_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: RefreshInterval_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return RefreshInterval_clock_PHASE & RefreshInterval_clock_PHASE_MASK;
}
