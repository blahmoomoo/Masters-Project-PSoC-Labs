...
/*******************************************************************************
* Function Name: VoiceB_clock_Start
********************************************************************************
*
* Summary:
...
void VoiceB_clock_Start(void) 
{
    ...
    VoiceB_clock_CLKEN |= VoiceB_clock_CLKEN_MASK;
	VoiceB_clock_CLKSTBY |= VoiceB_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: VoiceB_clock_Stop
********************************************************************************
*
* Summary:
...
void VoiceB_clock_Stop(void) 
{
    ...
    VoiceB_clock_CLKEN &= (uint8)(~VoiceB_clock_CLKEN_MASK);
	VoiceB_clock_CLKSTBY &= (uint8)(~VoiceB_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: VoiceB_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((VoiceB_clock_CLKEN & VoiceB_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = VoiceB_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = VoiceB_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* VoiceB_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(VoiceB_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(VoiceB_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: VoiceB_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: VoiceB_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = VoiceB_clock_GetSourceRegister();
    uint16 oldDivider = VoiceB_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = VoiceB_clock_CLKEN & VoiceB_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(VoiceB_clock_DIV_PTR, clkDivider);
                VoiceB_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                VoiceB_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(VoiceB_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((VoiceB_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: VoiceB_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(VoiceB_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: VoiceB_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    VoiceB_clock_MOD_SRC |= modeBitMask & (uint8)VoiceB_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: VoiceB_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    VoiceB_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(VoiceB_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: VoiceB_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return VoiceB_clock_MOD_SRC & (uint8)(VoiceB_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: VoiceB_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = VoiceB_clock_GetDividerRegister();
    uint8 oldSrc = VoiceB_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        VoiceB_clock_MOD_SRC |= CYCLK_SSS;
        VoiceB_clock_MOD_SRC =
            (VoiceB_clock_MOD_SRC & (uint8)(~VoiceB_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: VoiceB_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return VoiceB_clock_MOD_SRC & VoiceB_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: VoiceB_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    VoiceB_clock_PHASE = clkPhase & VoiceB_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: VoiceB_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return VoiceB_clock_PHASE & VoiceB_clock_PHASE_MASK;
}
